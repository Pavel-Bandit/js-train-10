...
function customObjectFromEntries(entries) {
  ...
  let result = Object.fromEntries(mapEntries);
  return result;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let objAsign = Object.assign(obj1, obj2);
  Object.freeze(objAsign);
  if (Object.isFrozen(objAsign)) {
    return objAsign;
  } else if (!Object.isFrozen(objAsign)) {
    return "Error , Обєкт не було заморожено";
  }
}
...
function filterObjectsByKey(arr, key) {
  ...
  let result = arr.filter((filterObjectsByKey) => filterObjectsByKey[key]);
  return result;
}
...
function checkAndPreventExtensions(obj) {
  ...
  let exstensibleObj = Object.isExtensible(obj);
  if (exstensibleObj === true) {
